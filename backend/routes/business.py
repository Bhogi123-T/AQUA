...
def business_portal():
    ...
    def add_multi_curr(records, amount_key):
        for r in records:
            amt_str = str(r.get(amount_key, "0")).replace("₹", "").replace("$", "").replace(",", "").strip()
            try:
                amt = float(amt_str)
            except:
                amt = 0.0
            # Assume db stores INR based on existing logic
            usd_amt = amt / USD_TO_INR
            prices = get_global_prices(usd_amt)
            r['multi_currency'] = f"${prices['USD']} | €{prices['EUR']} | ₹{prices['INR']} | ¥{prices['JPY']}"
        return records
        
    orders_enriched = add_multi_curr(orders[-20:], 'total_inr')
    incoming_enriched = add_multi_curr(incoming_orders, 'total_inr')
    payments_enriched = add_multi_curr(payments[-20:], 'amount')
        
    return render_template("business_portal.html",
                           trans=trans, lang=lang,
                           orders=orders_enriched,  # pyre-ignore
                           incoming_orders=incoming_enriched,
                           payments=payments_enriched)  # pyre-ignore
