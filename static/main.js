...
async function checkLocationPermission() {
    ...
    try {
        // Check if geolocation is available or if we are in an insecure context (HTTP)
        const isInsecure = window.location.protocol === 'http:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1';

        if (!('geolocation' in navigator) || isInsecure) {
            if (isInsecure) console.warn('⚠️ Geolocation blocked on HTTP (Insecure Context)');
            const hasIPLoc = await getIPLocation();
            window.LOCATION_ENABLED = hasIPLoc;
            return hasIPLoc;
        }
        ...
        // Browser Geolocation failed (timeout or denied)
        ...
        window.LOCATION_ENABLED = false;
        // Only show notification if we couldn't even get IP location
...
function disableLiveFeaturesNoLocation() {
    console.log('📍 Location disabled - geo-features unavailable (telemetry still active if online)');
    // NOTE: We intentionally do NOT set ALLOW_LIVE_DATA = false here anymore.
    // Telemetry data is server-simulated and doesn't need GPS.
    // Only geo-specific features (map overlays, nearby experts) require location.
}
...
function initializeConnectionMonitor() {
    const updateConnectionStatus = async () => {
        ...
        // STEP 2: Check location permission in background (for geo-features only, not telemetry)
        checkLocationPermission(); // fire and forget - doesn't block telemetry

        // STEP 3: Live data only requires internet - telemetry is server-simulated
        if (navigator.onLine) {
